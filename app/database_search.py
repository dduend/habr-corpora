...
class ProcessingUserRequest:
    ...
    def parse_single_token(self, token):
        ...
        if token in self.pos_tags:  # Поиск по части речи
            ...
            if all(char.isascii() for char in token.strip('"').lower()):
                # Проверяем, что введённое пользователем слово — не полностью
                # написанное латиницей (при этом часть символов могут быть
                # латинскими, например "it-специалиста" — валидный запрос)
                error = 'Слово не может быть полностью написано латиницей! ' \
                        'Если же вы имели в виду тег, то среди доступных ' \
                        'его нет, проверьте инструкцию по поиску!'
                return error
    ...
    def parse_multipart_token(self, token):
        ...
        if token == '+':
            error = 'Некорректный ввод запроса: лишние пробелы вокруг "+"!'
            return error
        parts = token.split('+')
        if len(parts) > 3:
            error = 'В токене может быть макс. 3 элемента — лемма, POS-тег ' \
                    'и deprel-тег!'
            return error
        else:
            lemma = parts[0].lower()
            if len(parts) == 2:
                if parts[1] in self.pos_tags:
                    if all(char.isascii() for char in lemma):
                        error = 'Лемма в составном токене не может быть ' \
                                'полностью написана латиницей!'
                        return error
                    else:
                    ...
                    if all(char.isascii() for char in lemma):
                        error = 'Лемма в составном токене не может быть ' \
                                'полностью написана латиницей! Кроме того, ' \
                                'введённого тега среди доступных нет, ' \
                                'проверьте инструкцию по поиску!'
                        return error
                    else:
                        error = 'Введённого тега среди доступных нет, ' \
                                'проверьте инструкцию по поиску!'
                        return error
            else:
                if parts[1] in self.pos_tags and parts[2] in self.deprel_tags:
                    ...
                    if parts[1] not in self.pos_tags \
                            and parts[2] in self.deprel_tags:
                        if all(char.isascii() for char in lemma):
                            error = 'Лемма в составном токене не может быть ' \
                                    'полностью написана латиницей! Кроме ' \
                                    'того, введённого POS-тега среди ' \
                                    'доступных нет, проверьте инструкцию по ' \
                                    'поиску!'
                            return error
                        else:
                            error = 'Введённого POS-тега среди доступных ' \
                                    'нет, проверьте инструкцию по поиску!'
                            return error
                    elif parts[1] in self.pos_tags \
                            and parts[2] not in self.deprel_tags:
                        if all(char.isascii() for char in lemma):
                            error = 'Лемма в составном токене не может быть ' \
                                    'полностью написана латиницей! Кроме ' \
                                    'того, введённого deprel-тега среди ' \
                                    'доступных нет, проверьте инструкцию по ' \
                                    'поиску!'
                            return error
                        else:
                            error = 'Введённого deprel-тега среди доступных ' \
                                'нет, проверьте инструкцию по поиску!'
                            return error
                        ...
                        if all(char.isascii() for char in lemma):
                            error = 'Лемма в составном токене не может быть ' \
                                    'полностью написана латиницей! Кроме ' \
                                    'того, теги введены в неверном порядке: ' \
                                    'необходимо ввести сперва POS-, а затем ' \
                                    '— deprel-тег!'
                            return error
                        else:
                            error = 'Неверный порядок ввода тегов: ' \
                                    'необходимо ввести сперва POS-, а затем ' \
                                    '— deprel-тег!'
                            return error
                    else:
                        if all(char.isascii() for char in lemma):
                            error = 'Лемма в составном токене не может быть ' \
                                    'полностью написана латиницей! Кроме ' \
                                    'того, введённых тегов среди доступных ' \
                                    'нет, проверьте инструкцию по поиску!'
                            return error
                        else:
                            error = 'Введённых тегов среди доступных нет, ' \
                                'проверьте инструкцию по поиску!'
                            return error
